...
generic_ews <- function(timeseries, winsize = 50, detrending = c("no", "gaussian", 
  ...
  dev.new()
  par(mar = (c(0, 2, 0, 1) + 0.3), oma = c(7, 2, 3, 1), mfrow = c(5, 2))
  ...
  legend("bottomright", paste("Kendall tau=", round(KtAR$estimate, digits = 3)), 
         bty = "n")
  ...
  legend("bottomright", paste("Kendall tau=", round(KtACF$estimate, digits = 3)), 
         bty = "n")
  ...
  legend("bottomright", paste("Kendall tau=", round(KtRETURNRATE$estimate, digits = 3)), 
         bty = "n")
  legend("bottomleft", "return rate", bty = "n")
  plot(timeindex[mw:length(nsmY)], nDENSITYRATIO, ylab = "", xlab = "", type = "l", 
       xaxt = "n", las = 1, xlim = c(timeindex[1], timeindex[length(timeindex)]))
  legend("topright", paste("Kendall tau=", round(KtDENSITYRATIO$estimate, digits = 3)), 
         bty = "n")
  legend("topleft", "density ratio", bty = "n")
  plot(timeindex[mw:length(nsmY)], nSD, ylab = "", xlab = "", type = "l", xaxt = "n", 
       las = 1, xlim = c(timeindex[1], timeindex[length(timeindex)]))
  legend("bottomright", paste("Kendall tau=", round(KtSD$estimate, digits = 3)), 
         bty = "n")
  legend("topleft", "standard deviation", bty = "n")
  if (detrending == "no") {
    plot(timeindex[mw:length(nsmY)], nCV, ylab = "", xlab = "", type = "l", xaxt = "n", 
         las = 1, xlim = c(timeindex[1], timeindex[length(timeindex)]))
    legend("bottomright", paste("Kendall tau=", round(KtCV$estimate, digits = 3)), 
           bty = "n")
    legend("topright", "coefficient of variation", bty = "n")
  } else {
  ...
  legend("topright", paste("Kendall tau=", round(KtSK$estimate, digits = 3)), 
         bty = "n")
  mtext("time", side = 1, line = 2, cex = 0.8)
  plot(timeindex[mw:length(nsmY)], nKURT, type = "l", ylab = "", xlab = "", las = 1, 
       cex.lab = 1, xlim = c(timeindex[1], timeindex[length(timeindex)]))
  legend("topleft", "kurtosis", bty = "n")
  legend("topright", paste("Kendall tau=", round(KtKU$estimate, digits = 3)), 
         bty = "n")
  mtext("time", side = 1, line = 2, cex = 0.8)
  ...
  if (powerspectrum) {
    ...
    image(x = (spectfft$freq[2:length(spectfft$freq)]), y = (seq(1, ncol(nSPECT),
                                                                 by = 1)), log(nSPECT[2:length(spectfft$freq), ]), ylab = "",
          xlab = "", log = "x", xlim = c(spectfft$freq[2], spectfft$freq[length(spectfft$freq)]),
          col = topo.colors(20), xaxs = "i")
    filled.contour(x = (spectfft$freq[2:length(spectfft$freq)]), y = (seq(1, ncol(nSPECT),
                                                                   by = 1)), log(nSPECT[2:length(spectfft$freq), ]), col =  topo.colors(30))
      
#                                                                   by = 1)), log(nSPECT[2:length(spectfft$freq), ]), col =  topo.colors(25))
