...
PlotPotential <- function(res, title = "", xlab.text, ylab.text, cutoff = 0.5, plot.contours = TRUE, 
    ...
    #cols = rev(colorRampPalette('white')(24))
    p <- ggplot(df)
    p <- p + geom_tile(aes(x = bg.var, y = phylotype, z = potential, fill = potential)) + theme_bw()
    #p <- p + scale_fill_continuous(low = "blue", high = "red")
    p <- p + scale_fill_gradientn(colours = inferno(4),guide=FALSE,trans = 'reverse')
#    p <- p + scale_fill_gradientn(colours = inferno(4),trans = 'reverse')
    ...
    if (plot.contours) {
        if (!is.null(bins)) {
            ...
            p <- p + stat_contour(bins = bins, aes(x = bg.var, y = phylotype, z = potential, fill = potential)
                                  , show.legend = TRUE,size = 0.7,color="black")
